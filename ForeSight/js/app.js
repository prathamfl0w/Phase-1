(function () {
  ...
  if (!document.getElementById("modal")) {
    ...
    modal.innerHTML =
      '<div class="modal-content"><div class="modal-header"><h2 id="modal-title">Products</h2><button id="modal-close" type="button" aria-label="Close">&times;</button></div><div id="modal-list" class="modal-list-container"></div></div>';
  ...
  function render() {
    ...
    document.getElementById("stat-total").querySelector(".count").textContent =
      totalCount;
    document
      .getElementById("stat-reorder")
      .querySelector(".count").textContent = reorderCount;
    document.getElementById("stat-low").querySelector(".count").textContent =
      lowCount;
    document.getElementById("stat-ok").querySelector(".count").textContent =
      okCount;
    ...
    rowsData.sort((a, b) => {
      ...
      const daysA =
        a.forecast.daysOfStockLeft === null
          ? Infinity
          : a.forecast.daysOfStockLeft;
      const daysB =
        b.forecast.daysOfStockLeft === null
          ? Infinity
          : b.forecast.daysOfStockLeft;
      return daysA - daysB;
    });
    ...
    for (let i = 0; i < rowsData.length; i++) {
      ...
      const daysLeft =
        data.forecast.daysOfStockLeft === null
          ? "&mdash;"
          : data.forecast.daysOfStockLeft;
  ...
  document
    .getElementById("sale-category")
    .addEventListener("change", updateProductDropdown);
  ...
  function showModal(title, items) {
    ...
    if (items.length === 0) {
      listContainer.innerHTML =
        '<div style="text-align:center; opacity:0.6; padding: 1rem;">No items in this category</div>';
    } else {
      const grouped = {};
      items.forEach((item) => {
  ...
  document
    ...
    .addEventListener("click", function () {
      currentFilter = "REORDER_NOW";
      render();
      showModal(
        "🚨 Needs Reordering",
        lastRenderedRows.filter((r) => r.forecast.status === "REORDER_NOW"),
      );
    });
  document.getElementById("stat-low").addEventListener("click", function () {
    ...
    showModal(
      "⚠️ Running Low",
      lastRenderedRows.filter((r) => r.forecast.status === "LOW"),
    );
  });
  document.getElementById("stat-ok").addEventListener("click", function () {
    ...
    showModal(
      "✅ Healthy",
      lastRenderedRows.filter((r) => r.forecast.status === "OK"),
    );
  });
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const headerSaleLink = document.querySelector(
      ".quick-actions .button-secondary",
    );
    const headerProductLink = document.querySelector(
